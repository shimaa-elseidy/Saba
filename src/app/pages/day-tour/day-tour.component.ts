...
@Component({
  ...
  styleUrls: ['./day-tour.component.scss']
})
export class DayTourComponent implements OnInit {
  tours: Tour[] = [];
  filteredTours: Tour[] = [];
  ...
  loadTours(): void {
    this.tourService.getAllToursDay().subscribe({
      next: (tours: Tour[]) => {
        this.tours = tours;
        this.applyFilters();
      },
      error: (err) => {
        console.error('Error fetching Day tours:', err);
      }
    });
  }
